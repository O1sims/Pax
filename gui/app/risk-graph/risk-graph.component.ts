...
import { GroupInfoWindow, GroupInfoWindowData } from './modals/group-info/group.info.modal';
import { AssetInfoWindow, AssetInfoWindowData } from './modals/asset-info/asset.info.modal';
import { ThreatInfoWindow, ThreatInfoWindowData } from './modals/threat-info/threat.info.modal';
import { ActionWindow, ActionWindowData } from './modals/action-modal/courses.action.modal';
...
export class RiskGraphComponent implements OnInit {
  ...
  zoom = 15;
  systemCoordinates = L.latLng(0, 0);
  assetCoordinates = L.latLng(0, 0);
  missionCoordinates = {
    'latitude': 0,
    'longitude': 0
  };
  ...
  probabilityOfCompleteSuccess : any;
  totalResourcesDeployed : any;
  hardwareAffected = [];
  softwareAffected = [];
  totalTimeTaken : any;
  riskReduction : any;
  ...
  numberOfStagedActions : any;
  stagedActionRows : any;
  coursesOfActionSelected = [];

  allMissions:object[] = [];
  systemId:string;
  missionId:string = "Please select...";
  ...
  selectedCOA: string;
  selectedComparativeCOA = {};

  comparativeStatics = 0;
...
doughnutChartData = [0, 0, 0, 0, 0];
doughnutChartType = 'doughnut';
doughnutOptions = {
  'cutoutPercentage': 80
};

compareRestrictions:number = 0;
compareMissionTime:number = 0;
comparePersonnel:number = 0;
compareProbability:number = 0;

unattainableStrategies = [];

numberOfAssets = 0;
numberOfCyberAssets = 0;
numberOfPhysicalAssets = 0;
...
selectedTaskDependencies:string[] = [];

usedTaskId = 0;
taskSubmitted:number = 0;

hasTaskDependents = 0;
taskDependents:string = "";
tasksToDelete:string[] = [];

maxRiskColour:string;

selectedRiskGraphCOA:string = "Initial risk profile";
riskGraphCOAs:string[] = [
  "Initial risk profile"
];
riskGraphGenerated:number = 0;
assetRiskChanges:object[] = [];

private sub: any;
private googleLoaded: any;

viewGanttChart:number = 0;
viewDependencyTree:number = 1;

estimatedTimeFrame:number = 0;

tooFarAway:object = {};
tooFarAction:object = {};

missionLength:number = 720;

private ganttOptions;
private ganttData;
private ganttChart;
...
constructor(private route: ActivatedRoute,
  ...
  calculateEstimatedTime() {
    ...
    this.riskGraphService.getEstimatedTime(
      this.systemId,
      taskData
  ...
  drawGanttChart(taskData, missionData) {
    ...
    this.ganttData.addRows(taskDataToGanttData(
      taskData,
      missionData['hHour']
    ));
    ...
    this.ganttChart = this.createGanttChart(
      document.getElementById('chart_taskGantt')
    );
  ...
  extractTimeData(estimatedTimes, courseOfAction) {
    ...
    for (let i = 0; i < taskIds.length; i++) {
      timeData.push([
        ...
        this.customCandlestickTooltip(
          courseOfAction[i],
          estimatedTimes[taskIds[i]],
          estimatedTimes[taskIds[i]]['estimatedTime']
        )
      ]);
    };
    return timeData;
  ...
  drawCandlestickChart(estimatedTimes, courseOfAction) {
    ...
    this.candlestickData.addRows(this.extractTimeData(
      estimatedTimes,
      courseOfAction
    ));
    ...
    this.candlestickChart = this.createCandlestickChart(
      document.getElementById('chart_timeCandlestick')
    );
  ...
  createCandlestickChart(element:any):any {
  ...
  submitNewAction() {
    ...
    taskTime:object = {};
    missionLengthExceeded:number = 0;
    hHour:any;
    zHour:any;
    ...
    getMissionTime(missionData, taskData) {
      ...
      this.riskGraphService.getMissionTimeAssessment(
        this.systemId,
        missionTaskData
      ).subscribe(timeAssessment => {
    ...
    evaluateActions() {
      ...
      .subscribe(estimatedTimes => {
        ...
        setTimeout(() => {
          google.charts.setOnLoadCallback(() => this.drawCandlestickChart(
            estimatedTimes,
            effectList
          ));
        }, 1);
    ...
    addCOA() {
      ...
      onClickActionTable(task) {
        ...
        var parentTasks = this.findAllParents(
          task['dependencies'][0],
          this.actionRows
        );
      ...
      findAllChildren(taskId, taskData) {
        var childTasks:object[] = [];
        var allChildren:string[] = [taskId];
        for (let j = 0; j < taskData.length ; j++) {
          var toAdd:string[] = [];
          for (let i = 0; i < taskData.length; i++) {
            var dependencies:string[] = taskData[i]['dependencies'];
