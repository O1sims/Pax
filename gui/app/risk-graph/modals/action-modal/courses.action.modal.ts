...
export class ActionWindow implements ModalComponent<ActionWindowData>, OnInit {
  ...
  heatmapSpinner: number;

  taskView:number = 1;
  actionNavView:number = 0;
  dependencyView:number = 0;
  ...
  constructor(public dialog: DialogRef<ActionWindowData>,
    ...
    actionsView() {
      ...
      this.dependencyView = 0;
      this.taskView = 0;
      this.actionModalService.getTaskActions(
        'hostile',
        this.actionData['effect'],
        this.objectiveData['assetType']
      ).subscribe(actions => {
